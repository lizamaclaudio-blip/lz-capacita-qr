...
export default function EditCompanyModal({ open, company, onClose, onSaved }: Props) {
  ...
  async function loadParentOptions() {
    ...
    // Si existe company_type, preferimos HQ; si no, mostramos todas menos la actual.
  ...
  useEffect(() => {
    ...
    // si es sucursal, cargamos opciones de casa matriz
    if (t === "branch") {
      loadParentOptions();
    } else {
      setParents([]);
  ...
  async function save() {
    ...
    const token = await getToken();
    if (!token) {
      setErr("Sesión no encontrada. Vuelve a iniciar sesión.");
      return;
    }
    ...
    try {
      ...
      // solo mandamos logo_path si subimos algo
      ...
      if (!res.ok) {
        setErr(json?.error || "No se pudo guardar la empresa.");
        return;
      }

      setOk("✅ Empresa actualizada.");
      onSaved();
      // dejamos un segundo para que el usuario vea el ok
        ...
        <div className={styles.header}>
          <div>
            ...
            <div className={styles.sub}>
              Actualiza datos, contacto y logo. (Sin botón eliminar)
            </div>
          </div>
